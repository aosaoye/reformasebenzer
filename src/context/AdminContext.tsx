...
export function AdminProvider({ children, isAdmin }: { children: React.ReactNode, isAdmin: boolean }) {
    ...
    return (
        <AdminContext.Provider value={{ isEditing: isAdmin && isEditing, setIsEditing: handleSetIsEditing }}>
            {isAdmin && (
                <div 
                    className="fixed top-0 left-0 right-0 z-[500] p-3 flex items-center justify-between text-xs font-bold uppercase tracking-widest shadow-2xl border-b border-stone-850"
